...
def should_succeed_with_validation_failure(
    ...
    with patch("backend.check_stac_metadata.utils.processing_assets_model_with_meta"), patch(
        "backend.parameter_store.SSM_CLIENT"
    ), patch("backend.validation_results_model.validation_results_model_with_meta"):
        lambda_handler(
            {
...
def should_report_duplicate_asset_names(validation_results_factory_mock: MagicMock) -> None:
    ...
    metadata = (
        ...
        f' "{STAC_EXTENT_KEY}": {{'
        f'"{STAC_EXTENT_SPATIAL_KEY}": {{"{STAC_EXTENT_BBOX_KEY}": [[-180, -90, 180, '
        f"90]]}},"
...
def should_validate_given_url(
    ...
    with patch.dict(environ, {AWS_DEFAULT_REGION_KEY: any_region_name()}), patch(
        "backend.parameter_store.SSM_CLIENT"
    ), patch("backend.validation_results_model.validation_results_model_with_meta"):
        # When
        lambda_handler(
