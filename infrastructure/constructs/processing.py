        ...
        *,
        ...
        check_files_checksums_single_task = BatchSubmitJobTask(
            ...
            container_overrides_command=[
                f"--dataset-id=Ref::{DATASET_ID_KEY}",
                f"--version-id=Ref::{VERSION_ID_KEY}",
                f"--first-item=Ref::{FIRST_ITEM_KEY}",
                f"--assets-table-name=Ref::{ASSETS_TABLE_NAME_KEY}",
                f"--results-table-name=Ref::{RESULTS_TABLE_NAME_KEY}",
                f"--s3-role-arn=Ref::{S3_ROLE_ARN_KEY}",
            ],
        )
        array_size = int(
